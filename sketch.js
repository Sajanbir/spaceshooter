...
for(var j=0;j<alienbulletGroup.length;j++){
  if(playerbullet.isTouching(alienbulletGroup)){
    destroybullets(playerbullet[i],alienbulletGroup[j])
}
}
...
for(var i=0;i<playerbullet.length;i++){
  for(var j=0;j<alienGroup.length;j++){
    if(playerbullet.isTouching(alienGroup)){
      destroyaliens(playerbullet[i],alienGroup[j])
  }
  }
